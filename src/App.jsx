...
/* ---------- 漂着物カタログ ---------- */
const CATALOG = [
  { id: "maru_ishi", cat: "石", name: "丸い石", rarity: 1, poem: "波に磨かれて、角がとれた。手のひらに、ちょうどいい重さ。" },
  { id: "shima_ishi", cat: "石", name: "縞模様の石", rarity: 2, poem: "幾千年の地層が、一本の縞になった。" },
  { id: "shiro_ishi", cat: "石", name: "白い石", rarity: 2, poem: "月のかけらのように、砂の上で光っていた。" },
...
function advanceShore(stored, now, weather) {
  ...
  if (items === null) {
    ...
    items = [];
    for (let i = 0; i < FIRST_VISIT; i++) items.push(newShoreItem(weather));
  ...
  for (let i = 0; i < n; i++) items.push(newShoreItem(weather));
  if (n > 0) dayCount += n;
...
function newShoreItem(weather) {
  return { id: rollItem(weather), seed: (Math.random() * 1e9) | 0 };
}
...
function drawItem(ctx, id, seed, s) {
  ...
  if (def.cat === "石") {
    ...
    if (id === "shiro_ishi") { g.addColorStop(0, "#f4efe3"); g.addColorStop(1, "#cfc7b4"); }
    else { g.addColorStop(0, "#a49a8b"); g.addColorStop(1, "#6e675c"); }
    ctx.fillStyle = g;
    ...
    if (id === "shima_ishi") {
      ...
      ctx.strokeStyle = "rgba(240,235,222,0.75)"; ctx.lineWidth = 1.6 * s;
      ...
      ctx.fillStyle = id === "shiro_ishi" ? "rgba(190,182,165,0.5)" : "rgba(235,228,215,0.4)";
      const specks = 4 + Math.floor(rnd() * 5);
      for (let i = 0; i < specks; i++) {
        ctx.beginPath(); ctx.arc((rnd() - 0.5) * 16 * s, (rnd() - 0.5) * 11 * s, (0.4 + rnd() * 0.7) * s, 0, Math.PI * 2); ctx.fill();
      }
    }
  } else if (def.cat === "貝殻") {
    if (id === "makigai") {
...
function BeachScene({ beach, collection, finds, beachNames, onCollect, onLeave }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!alive) return;
      const advanced = advanceShore(stored, Date.now(), weatherRef.current);
  ...
  useEffect(() => {
    ...
    function maybeWashUp() {
      ...
      const rec = newShoreItem(weatherRef.current);
